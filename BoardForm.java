...
public class BoardForm extends JFrame implements ActionListener {
	// 기존 헤더 관련 컴포넌트
	...
	public BoardForm() {
		 ...
		 // 로그인된 관리자인지 또는 회원으로 로그인되었는지 세션에서 확인
		...
		titleLabel = new JLabel("게시글 작성", JLabel.CENTER);
		...
		menuLabel.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				PopupMenuManager.showPopupMenu(menuLabel, BoardForm.this);
			}
		});
		...
		// ── 본문 영역 (게시글 입력 폼) ─────────────────────────────
		// formPanel: 전체 입력 폼을 담는 패널 (수직 정렬)
		JPanel formPanel = new JPanel();
		formPanel.setLayout(new BoxLayout(formPanel, BoxLayout.Y_AXIS));
		formPanel.setOpaque(false); // 배경 투명하게 설정
		...
		titleInputPanel.setOpaque(false); // 배경 투명하게 설정
		...
		// 플레이스홀더 텍스트 설정
		String placeholder = "  35자 이내로 작성해 주세요";
		...
		((AbstractDocument) titleField.getDocument()).setDocumentFilter(new DocumentFilter() {
			...
			public void replace(FilterBypass fb, int offset, int length, String text, AttributeSet attrs)
					throws BadLocationException {
				if (fb.getDocument().getLength() - length + text.length() <= 35) {
		...
		contentInputPanel.setOpaque(false); // 배경 투명하게 설정
        ...
        // 등록 버튼 액션 리스너 - 로그인된 세션에서 작성자 ID 가져오기
        submitButton.addActionListener(e -> {
            ...
            if (titleText.isEmpty() || contentText.isEmpty()) {
            	...
            	if (UserSession.getInstance().getCurrentAdmin() != null) {
            	    managerId = UserSession.getInstance().getCurrentAdmin().getManager_Id();
            	} else if (UserSession.getInstance().getCurrentUser() != null) {
            	    memberId = UserSession.getInstance().getCurrentUser().getMember_Id();
            	}
            	BoardMgr boardMgr = new BoardMgr();
            	boolean success = boardMgr.insertBoard(titleText, contentText, memberId, managerId);
		...
		cancelButton.addActionListener(e -> {
			...
			if (response == JOptionPane.YES_OPTION) {
				...
				contentArea.setText("");
				dispose(); // 현재 창 종료
				new BoardPageAWT(null); // BoardPageAWT 창 띄우기
			}
		});
